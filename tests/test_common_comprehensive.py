"""Comprehensive tests for bot.handlers.common — mocked Telegram API."""
import pytest
...
def make_message(text="/start", user_id=123):
    msg = AsyncMock()
    msg.text = text
...
def make_callback(data="lang:en", user_id=123):
    call = AsyncMock()
    call.data = data
...
class TestHelp:
    ...
    async def test_cmd_help(self):
        ...
        with patch("bot.handlers.common.db.get_lang", new_callable=AsyncMock, return_value="en"), \
             patch("bot.handlers.common.t", return_value="Help text"):
            await cmd_help(msg)
            msg.answer.assert_called()
